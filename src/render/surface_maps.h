// Shared 2D bilinear surface maps (Embree Exit to Diffuse dest + OptiX).
// Must not include render/shading.h — MaterialX procedurals hang the OptiX
// ETD / shade megakernels. Pref / noise / autobump stay CPU-only on the
// regular path; ETD dest uses this header on both backends.
#pragma once
...
SR_INL SR_HD Material evaluateSurfaceMaps(const SceneView& scene, const Material& base, Vec2 uv, Vec3& ns) {
    Material mat = base;
