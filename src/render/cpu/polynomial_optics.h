...
struct PolynomialOpticsCamera {
    ...
    // lensU/V in [0,1]. On success returns camera-space origin/direction in metres
    // (already flipped to look down -Z like the thin-lens path). Caller applies cameraToWorld.
    bool generateRayCameraSpace(double sensorXmm, double sensorYmm, float lensU, float lensV, Rng& rng,
                                Vec3& originCam, Vec3& dirCam) const;
};
