...
public:
    ...
    void addSplat(int x, int y, Vec3 v) {
        ...
        std::atomic<float>* px = splat_.get() + (size_t(y) * size_t(width_) + size_t(x)) * 3;
        for (int c = 0; c < 3; ++c) {
            const float add = c == 0 ? v.x : (c == 1 ? v.y : v.z);
            float cur = px[c].load(std::memory_order_relaxed);
    ...
    long splatPaths() const { return splatPaths_.load(std::memory_order_relaxed); }

    Vec3 splatPixel(int x, int y, float invPaths) const {
        if (!splat_ || invPaths <= 0.0f) return Vec3(0.0f);
        const std::atomic<float>* px = splat_.get() + (size_t(y) * size_t(width_) + size_t(x)) * 3;
        return Vec3(px[0].load(std::memory_order_relaxed), px[1].load(std::memory_order_relaxed),
                    px[2].load(std::memory_order_relaxed)) *
               invPaths;
    }
    ...
    Vec3 resolvePixel(int x, int y) const {
        ...
        return base + splatPixel(x, y, paths > 0 ? 1.0f / float(paths) : 0.0f);
    }
...
private:
    ...
    std::unique_ptr<std::atomic<float>[]> splat_;  // 3 floats per pixel
    std::atomic<long> splatPaths_{0};
