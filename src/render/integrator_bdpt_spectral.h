...
inline Vec3 traceRadianceBdptSpectral(
    ...
    if (photonCaustics) {
        for (int t = 2; t <= nEye; ++t) {
            ...
            if (t > 2) c = clampSpectrumIndirect(c, effectiveClampDirect(settings));
    ...
    for (int t = 2; t <= nEye; ++t) {
        ...
        if (l.type == kLightDome || l.type == kLightDistant) {
            LightSample ls;
            if (!sampleLight(scene, li, E.p, rng.nextFloat(), rng.nextFloat(), ls) ||
    ...
    for (int t = 2; t <= nEye; ++t) {
        ...
        for (int s = 2; s <= nLight; ++s) {
            ...
            c = clampSpectrumIndirect(c, effectiveClampDirect(settings));
