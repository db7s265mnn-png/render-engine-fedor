...
// Hero-λ fog (arrays). Same walk as sampleHeterogeneousFogSpectral on Embree.
SR_INL SR_HD bool fogInteractWl(const MediumData& medium, float densityScale, float occupancy, float tHit,
                               Rng& rng, float* throughput, const float* lambda, int n, MediumSample& out) {
    const float dens = srMax(0.0f, occupancy) * densityScale;
    float sigA[kMaxSpectrumSamples];
    float sigS[kMaxSpectrumSamples];
    specUpsampleLinear(medium.sigmaA * dens, lambda, n, sigA);
    specUpsampleLinear(medium.sigmaS * dens, lambda, n, sigS);
    float majorant = 0.0f;
    for (int i = 0; i < n; ++i) majorant = srMax(majorant, sigA[i] + sigS[i]);
    ...
    if (uMode < pA + pS) {
        ...
        for (int i = 0; i < n; ++i) throughput[i] *= sigS[i] / sHero;
        out.t = tHit;
        out.scattered = true;
        return true;
    }
...
SR_INL SR_HD bool fogCollideWl(const MediumData& medium, float densityScale, float occupancy, float majorant,
                              float tHit, Rng& rng, float* throughput, const float* lambda, int n,
                              MediumSample& out) {
    const float dens = srMax(0.0f, occupancy) * densityScale;
    float sigT[kMaxSpectrumSamples];
    specUpsampleLinear((medium.sigmaA + medium.sigmaS) * dens, lambda, n, sigT);
    float stHero = 0.0f;
    for (int i = 0; i < n; ++i) stHero = srMax(stHero, sigT[i]);
    if (rng.nextFloat() >= stHero / srMax(majorant, 1e-12f)) return false;
    return fogInteractWl(medium, densityScale, occupancy, tHit, rng, throughput, lambda, n, out);
}

template <typename Grid>
SR_INL SR_HD MediumSample sampleHeterogeneousFogWl(const Grid& grid, const MediumData& medium, Vec3 origin,
    ...
    for (int iter = 0; iter < kNullCollisionMaxIters && t < tMax; ++iter) {
        ...
        while (iter < kNullCollisionMaxIters) {
            ...
            if (tCtrl <= tRes) {
                fogInteractWl(medium, densityScale, minD, tCtrl, rng, throughput, lambda, n, out);
                return out;
            }
            const float occ = clampf(grid.sampleOcc(origin + direction * tRes), minD, maxD);
            const float pReal = (st0 * occ - muC) / srMax(residualMaj, 1e-12f);
            ++iter;
            if (rng.nextFloat() >= clampf(pReal, 0.0f, 1.0f)) continue;
            fogInteractWl(medium, densityScale, occ, tRes, rng, throughput, lambda, n, out);
            return out;
        }
        if (leftCell) continue;
        break;
...
SR_INL SR_HD MediumSample sampleMediumHomogeneousWl(const MediumData& m, float tMax, Rng& rng,
    ...
    float sigA[kMaxSpectrumSamples];
    float sigS[kMaxSpectrumSamples];
    specUpsampleLinear(mediumSigmaA(m), lambda, n, sigA);
    specUpsampleLinear(mediumSigmaS(m), lambda, n, sigS);
    float majorant = 0.0f;
    for (int i = 0; i < n; ++i) majorant = srMax(majorant, sigA[i] + sigS[i]);
