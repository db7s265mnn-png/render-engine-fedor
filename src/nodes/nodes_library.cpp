    ...
    explicit RenderSettingsNode(const QString& name) : Node("rendersettings", name) {
        ...
        addParameter(Parameter::makeMenu("causticsenginegpu", "Caustics Engine (GPU)",
                         ...
                         .withTooltip("OptiX half of XPU (Render Device = GPU uses the Caustics "
                                      ...
                                      "tracing SDS on the floor, plus eye-path Newton MNEE "
                                      "(Hanika 2015) in a dedicated OptiX pipeline. MNEE runs "
                                      "from the first delta-glass hit to the light (the lamp "
                                      "through the object) and from a specular-prefix floor hit "
                                      "(glass-blocked NEE). Depth-0 floor stays with LT. Newton "
                                      "is not compiled into shade / path_tail.\n"
