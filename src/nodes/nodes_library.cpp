    ...
    explicit RenderSettingsNode(const QString& name) : Node("rendersettings", name) {
        ...
        addParameter(Parameter::makeMenu("causticsenginegpu", "Caustics Engine (GPU)",
                         ...
                         .withTooltip("OptiX half of XPU (Render Device = GPU uses the Caustics "
                                      ...
                                      "tracing SDS (floor) and L S* C (specular chain that aims "
                                      "at the film — the lamp seen through glass), plus eye-path "
                                      "Newton MNEE (Hanika 2015) in a dedicated OptiX pipeline — "
                                      "same lazy glass-blocked NEE upgrade as CPU integrator_mnee. "
                                      "Newton is not compiled into shade / path_tail.\n"
