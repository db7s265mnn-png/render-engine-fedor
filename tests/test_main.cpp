...
void testPolynomialOpticsCamera() {
    ...
    Node* camera = graph.createNode("camera", "camera1");
    Node* settings = graph.createNode("rendersettings", "rendersettings1");
    check(camera && settings, "optics graph nodes created");
    if (camera) {
        camera->setParameterValue("opticalmodel", 1);
        camera->setParameterValue("lensmodel", 19);
        camera->setParameterValue("chromatic", true);
        camera->setParameterValue("wavelength", 480.0);
        camera->setParameterValue("fstop", 2.0);
        camera->setParameterValue("focusdistance", 2.5);
        graph.connectNodes(camera, settings, 0);
        graph.setDisplayNode(settings);
        CookContext context;
        StagePtr stage = graph.cookDisplay(context);
        check(stage != nullptr, "optics camera cooks");
        ScenePtr scene = stage ? stage->toScene() : nullptr;
        check(scene != nullptr, "optics camera toScene");
        if (scene) {
            check(scene->camera.opticalModel == 1, "opticalModel cooks to scene");
            check(scene->camera.lensModel == 19, "lensModel cooks to scene");
            check(scene->camera.chromaticAberration == 1, "chromatic aberration cooks to scene");
            checkNear(scene->camera.opticalWavelengthNm, 480.0f, 1e-3f, "wavelength cooks to scene");
        }
    }
}

void testPolynomialOpticsPathTracer() {
    std::printf("polynomial-optics-path-tracer\n");
    auto build = [](int integrator, int chromatic) {
        ScenePtr scene = std::make_shared<Scene>();
        MeshPtr wall = makeBoxMesh(Vec3(6.0f, 6.0f, 0.2f));
        wall->ensureRenderTriangles();
        wall->computeBounds();
        const int meshIndex = scene->addMesh(wall);
        Material mat;
        mat.baseColor = Vec3(0.85f);
        mat.roughness = 0.6f;
        mat.emissionColor = Vec3(1.0f);
        mat.emissionStrength = 2.5f;
        ...
        scene->camera.cameraToWorld =
            lookAtMatrix(Vec3(0.0f, 0.0f, 3.0f), Vec3(0.0f, 0.0f, 0.0f), Vec3(0.0f, 1.0f, 0.0f));
        scene->camera.opticalModel = 1;
        scene->camera.lensModel = 19;  // cooke speed panchro 50mm
        scene->camera.sensorWidth = 36.0f;
        scene->camera.fStop = 2.0f;
        scene->camera.focusDistance = 3.0f;
        scene->camera.opticalWavelengthNm = 550.0f;
        scene->camera.chromaticAberration = chromatic;
        scene->cameraAuthored = true;
    ...
    auto stats = [](const Image& image, double& sum, double& center, double& corner, bool& finite,
                    double rgb[3]) {
        sum = 0.0;
        center = 0.0;
        corner = 0.0;
        ...
        if (nCenter > 0) center /= double(nCenter);
        if (nCorner > 0) corner /= double(nCorner);
    };

    {
        RenderSession session;
        session.setScene(build(kIntegratorPathTracer, 0));
        session.start();
        session.waitForCompletion();
        const Image image = session.linearImage();
        double sum = 0, center = 0, corner = 0, rgb[3] = {};
        bool finite = true;
        stats(image, sum, center, corner, finite, rgb);
        check(finite, "PT + Polynomial Optics output is finite");
        check(sum > 0.0, "PT + Polynomial Optics produces light");
        check(center > 0.0, "PT + Polynomial Optics lights the image centre");
        std::printf("  PT optics sum=%.3f center=%.4f corner=%.4f\n", sum, center, corner);
    }

    {
        RenderSession session;
        session.setScene(build(kIntegratorPathTracer, 1));
        session.start();
        session.waitForCompletion();
        const Image image = session.linearImage();
        double sum = 0, center = 0, corner = 0, rgb[3] = {};
        bool finite = true;
        stats(image, sum, center, corner, finite, rgb);
        check(finite, "PT + Polynomial Optics + CA output is finite");
        check(sum > 0.0, "PT + Polynomial Optics + CA produces light");
        check(rgb[0] > 0.0 && rgb[1] > 0.0 && rgb[2] > 0.0,
              "PT lens chromatic aberration deposits all RGB hero channels");
        std::printf("  PT optics CA sum=%.3f rgb=(%.2f,%.2f,%.2f)\n", sum, rgb[0], rgb[1], rgb[2]);
    }

    {
        // BDPT must stay on Thin Lens (no crash / black frame) when Optics is on.
        RenderSession session;
        session.setScene(build(kIntegratorBdpt, 0));
        session.start();
        session.waitForCompletion();
        const Image image = session.linearImage();
        double sum = 0, center = 0, corner = 0, rgb[3] = {};
        bool finite = true;
        stats(image, sum, center, corner, finite, rgb);
        check(finite, "BDPT + Optics-on-camera stays finite (Thin Lens fallback)");
        check(sum > 0.0, "BDPT + Optics-on-camera still produces light");
    }
}
...
int main() {
    ...
    testPolynomialOpticsPathTracer();
    testEnvironment();
    testDomeHdrLoad();
